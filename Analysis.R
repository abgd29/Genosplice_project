...
gene_counts = read.table("./GSE229613_gene_count_1.txt", header = TRUE)
...
ggplot(top20, aes(x = fct_reorder(Description, GeneRatio_numeric),
  ...
  theme(legend.key.size = unit(1.5, "lines")) +  # ↑ taille de la légende
  guides(color = guide_legend(override.aes = list(size = 6)))  # ↑ taille des pastilles couleur
